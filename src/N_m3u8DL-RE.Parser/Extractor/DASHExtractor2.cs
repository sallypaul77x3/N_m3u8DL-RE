...
internal class DASHExtractor2 : IExtractor
{
    ...
    private string ExtendBaseUrl(XElement element, string oriBaseUrl)
    {
        ...
        if (target != null) oriBaseUrl = ParserUtil.CombineURL(oriBaseUrl, target.Value);
    ...
    public async Task<List<StreamSpec>> ExtractStreamsAsync(string rawText)
    {
        ...
        foreach (var period in periods)
        {
            ...
            foreach (var adaptationSet in adaptationSets)
            {
                ...
                var mimeType = adaptationSet.Attribute("contentType")?.Value ??
                               adaptationSet.Attribute("mimeType")?.Value;
                ...
                foreach (var representation in representations)
                {
                    ...
                    if (mimeType == null)
                        mimeType = representation.Attribute("contentType")?.Value ??
                    ...
                    streamSpec.Language = FilterLanguage(representation.Attribute("lang")?.Value ??
                                                         adaptationSet.Attribute("lang")?.Value);
                    streamSpec.FrameRate = frameRate ?? GetFrameRate(representation);
                    streamSpec.Resolution = representation.Attribute("width")?.Value != null
                    ...
                    //推测后缀名
                    var mType = representation.Attribute("mimeType")?.Value ??
                                adaptationSet.Attribute("mimeType")?.Value;
                    if (mType != null)
                    {
                    ...
                    //优化字幕场景识别
                    var role = representation.Elements().Where(e => e.Name.LocalName == "Role").FirstOrDefault() ??
                               adaptationSet.Elements().Where(e => e.Name.LocalName == "Role").FirstOrDefault();
                    if (role != null)
                    {
                    ...
                    if (timeShiftBufferDepth != null)
                        streamSpec.Playlist.RefreshIntervalMs =
                            XmlConvert.ToTimeSpan(timeShiftBufferDepth).TotalMilliseconds / 2;

                    //读取声道数量
                    var audioChannelConfiguration = adaptationSet.Elements().Concat(representation.Elements())
                        .Where(e => e.Name.LocalName == "AudioChannelConfiguration").FirstOrDefault();
                    if (audioChannelConfiguration != null)
                        streamSpec.Channels = audioChannelConfiguration.Attribute("value")?.Value;

                    //发布时间
                    if (!string.IsNullOrEmpty(publishTime)) streamSpec.PublishTime = DateTime.Parse(publishTime);


                    //第一种形式 SegmentBase
                    var segmentBaseElement = representation.Elements().Where(e => e.Name.LocalName == "SegmentBase")
                        .FirstOrDefault();
                    if (segmentBaseElement != null)
                    {
                        //处理init url
                        var initialization = segmentBaseElement.Elements()
                            .Where(e => e.Name.LocalName == "Initialization").FirstOrDefault();
                        if (initialization != null)
                        {
                            ...
                            if (sourceURL == null)
                            {
                                ...
                                (
                                    new MediaSegment
                                    {
                                        ...
                                        Url = segBaseUrl,
                                        Duration = XmlConvert
                                            .ToTimeSpan(periodDuration ?? mediaPresentationDuration ?? "PT0S")
                                            .TotalSeconds
                                    }
                                );
                            }
                            ...
                            {
                                var initUrl = ParserUtil.CombineURL(segBaseUrl,
                                    initialization.Attribute("sourceURL")?.Value!);
                    ...
                    var segmentList = representation.Elements().Where(e => e.Name.LocalName == "SegmentList")
                        .FirstOrDefault();
                    if (segmentList != null)
                    {
                        var durationStr = segmentList.Attribute("duration")?.Value;
                        //处理init url
                        var initialization = segmentList.Elements().Where(e => e.Name.LocalName == "Initialization")
                            .FirstOrDefault();
                        if (initialization != null)
                        {
                            var initUrl = ParserUtil.CombineURL(segBaseUrl,
                        ...
                        for (var segmentIndex = 0; segmentIndex < segmentURLs.Count(); segmentIndex++)
                        {
                    ...
                    var segmentTemplateElementsOuter =
                        adaptationSet.Elements().Where(e => e.Name.LocalName == "SegmentTemplate");
                    //representation中的segmentTemplate
                    var segmentTemplateElements =
                        representation.Elements().Where(e => e.Name.LocalName == "SegmentTemplate");
                    if (segmentTemplateElements.Any() || segmentTemplateElementsOuter.Any())
                    {
                        //优先使用最近的元素
                        var segmentTemplate = (segmentTemplateElements.FirstOrDefault() ??
                                               segmentTemplateElementsOuter.FirstOrDefault())!;
                        var segmentTemplateOuter = (segmentTemplateElementsOuter.FirstOrDefault() ??
                                                    segmentTemplateElements.FirstOrDefault())!;
                        ...
                        var presentationTimeOffsetStr = segmentTemplate.Attribute("presentationTimeOffset")?.Value ??
                                                        segmentTemplateOuter.Attribute("presentationTimeOffset")
                                                            ?.Value ?? "0";
                        //timesacle
                        var timescaleStr = segmentTemplate.Attribute("timescale")?.Value ??
                                           segmentTemplateOuter.Attribute("timescale")?.Value ?? "1";
                        var durationStr = segmentTemplate.Attribute("duration")?.Value ??
                                          segmentTemplateOuter.Attribute("duration")?.Value;
                        var startNumberStr = segmentTemplate.Attribute("startNumber")?.Value ??
                                             segmentTemplateOuter.Attribute("startNumber")?.Value ?? "1";
                        //处理init url
                        var initialization = segmentTemplate.Attribute("initialization")?.Value ??
                                             segmentTemplateOuter.Attribute("initialization")?.Value;
                        if (initialization != null)
                        {
                        ...
                        var mediaTemplate = segmentTemplate.Attribute("media")?.Value ??
                                            segmentTemplateOuter.Attribute("media")?.Value;
                        var segmentTimeline = segmentTemplate.Elements()
                            .Where(e => e.Name.LocalName == "SegmentTimeline").FirstOrDefault();
                        if (segmentTimeline != null)
                        {
                            ...
                            foreach (var S in Ss)
                            {
                                ...
                                if (_repeatCount < 0)
                                    ...
                                    _repeatCount =
                                        (long)Math.Ceiling(
                                            XmlConvert.ToTimeSpan(periodDuration ?? mediaPresentationDuration ?? "PT0S")
                                                .TotalSeconds * timescale / _duration) - 1;
                        ...
                        else
                        {
                            ...
                            var totalNumber = (long)Math.Ceiling(
                                XmlConvert.ToTimeSpan(periodDuration ?? mediaPresentationDuration ?? "PT0S")
                                    .TotalSeconds * timescale / duration);
                            ...
                            if (totalNumber == 0 && isLive)
                            {
                                ...
                                //可用时间+偏移量
                                var offsetMs =
                                    TimeSpan.FromMilliseconds(Convert.ToInt64(presentationTimeOffsetStr) / 1000);
                            ...
                            for (long index = startNumber, segIndex = 0;
                    ...
                    if (streamSpec.Playlist.MediaParts[0].MediaSegments.Count == 0)
                        ...
                        (
                            new MediaSegment
                            {
                                Index = 0,
                                Url = segBaseUrl,
                                Duration = XmlConvert.ToTimeSpan(periodDuration ?? mediaPresentationDuration ?? "PT0S")
                                    .TotalSeconds
                            }
                        );
                    //解析加密信息  优先手动设置
                    ...
                    var _index = streamList.FindIndex(_f =>
                        _f.PeriodId != streamSpec.PeriodId && _f.GroupId == streamSpec.GroupId &&
                    ...
                    if (_index > -1)
                    {
                        ...
                        else
                        {
                            ...
                            if (url1 != url2)
                            {
                                var startIndex = streamList[_index].Playlist!.MediaParts.Last().MediaSegments.Last()
                                    .Index + 1;
                                var enumerator = streamSpec.Playlist.MediaParts[0].MediaSegments.GetEnumerator();
                                while (enumerator.MoveNext()) enumerator.Current.Index += startIndex;
                            ...
                            {
                                streamList[_index].Playlist!.MediaParts.Last().MediaSegments.Last().Duration +=
                                    streamSpec.Playlist.MediaParts[0].MediaSegments.Sum(x => x.Duration);
                            }
                        }
                    }
                    else
                    {
                        ...
                        //分片默认后缀m4s
                        if (streamSpec.MediaType != MediaType.SUBTITLES && (streamSpec.Extension == null ||
                                                                            streamSpec.Playlist.MediaParts.Sum(x =>
                                                                                x.MediaSegments.Count) >
                                                                            1)) streamSpec.Extension = "m4s";
    ...
    {
        for (var i = 0; i < streamSpecs.Count; i++)
        {
            ...
            if (playlist != null)
            {
                ...
                for (var ii = 0; ii < playlist!.MediaParts.Count; ii++)
                {
                    ...
                    for (var iii = 0; iii < part.MediaSegments.Count; iii++)
